// todo fix url
export const url = "https://chess-mates-server.herokuapp.com";
export const uci = ["a", "b", "c", "d", "e", "f", "g", "h"]
export const black = "rgba(181,99,50,255)";
export const white = "rgba(246,221,185,255)";
...
export const FEN = (fen) => {
    let pos, char;
    const fen_board = [], rows = fen.split("/");;
    for (let i = 0; i < 8; i++) {
        fen_board[i] = Array(8).fill(null);
        pos = 0;
        for (let j = 0; j < 8; pos++) {
            char = rows[i].charAt(pos);
            if ('1' <= char && char <= '8') {
                j += parseInt(char);
            }
            else {
